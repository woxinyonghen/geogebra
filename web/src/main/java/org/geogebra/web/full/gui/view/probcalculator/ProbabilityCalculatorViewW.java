...
public class ProbabilityCalculatorViewW extends ProbabilityCalculatorView
	...
	protected void updateGUI() {
		...
		for (int i = 0; i < maxParameterCount; ++i) {
			...
			if (hasParm) {
				...
				fldParameterArray[i].setText(parameters[i].getRedefineString(false, false, StringTemplate.editorTemplate));
	...
	private void updateLowHigh() {
		...
		fldLow.setText(low.getRedefineString(false, false, StringTemplate.editorTemplate));
		fldHigh.setText(high.getRedefineString(false, false, StringTemplate.editorTemplate));
	}
	...
	void doTextFieldActionPerformed(MathTextFieldW source, boolean intervalCheck) {
		...
		if (!"".equals(inputText)) {
			...
			GeoNumeric numericValue = nv instanceof GeoNumeric ?
					(GeoNumeric) nv : new GeoNumeric(cons, Double.NaN);
	...
	private void checkBounds(GeoNumeric value, boolean intervalCheck, boolean high) {
